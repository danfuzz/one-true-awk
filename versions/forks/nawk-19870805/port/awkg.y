...
ppattern:
	...
	| NOT ppattern
		{ $$ = op1(NOT, op2(NE,$2,nullnode)); }
	| ppattern MATCHOP reg_expr	{ $$ = op3($2, NIL, $1, makedfa($3, 0)); }
	| ppattern MATCHOP ppattern
		{ if (constnode($3))
			$$ = op3($2, NIL, $1, makedfa(strnode($3), 0));
		  else
			$$ = op3($2, (Node *)1, $1, $3); }
	...
	| reg_expr
		{ $$ = op3(MATCH, NIL, rectonode(), makedfa($1, 0)); }
	| term
	;
...
pattern:
	...
	| pattern MATCHOP reg_expr	{ $$ = op3($2, NIL, $1, makedfa($3, 0)); }
...
term:
	...
	| MATCHFCN '(' pattern comma reg_expr ')'
		{ $$ = op3(MATCHFCN, NIL, $3, makedfa($5, 1)); }
	| MATCHFCN '(' pattern comma pattern ')'
		{ if (constnode($5))
			$$ = op3(MATCHFCN, NIL, $3, makedfa(strnode($5), 1));
		  else
	...
	| SPLIT '(' pattern comma varname comma reg_expr ')'    /* const /regexp/ */
		{ $$ = op4(SPLIT, $3, makearr($5), makedfa($7, 1), (Node *)REGEXPR); }
	| SPLIT '(' pattern comma varname ')'
		{ $$ = op4(SPLIT, $3, makearr($5), NIL, (Node*)STRING); }  /* default */
	| SPRINTF '(' patlist ')'	{ $$ = op1($1, $3); }
	| STRING	 		{ $$ = valtonode($1, CCON); }
	| subop '(' reg_expr comma pattern ')'
		{ $$ = op4($1, NIL, makedfa($3, 1), $5, rectonode()); }
	| subop '(' pattern comma pattern ')'
		{ if (constnode($3))
			$$ = op4($1, NIL, makedfa(strnode($3), 1), $5, rectonode());
		  else
			$$ = op4($1, (Node *)1, $3, $5, rectonode()); }
	| subop '(' reg_expr comma pattern comma pattern ')'
		{ $$ = op4($1, NIL, makedfa($3, 1), $5, $7); }
	| subop '(' pattern comma pattern comma pattern ')'
		{ if (constnode($3))
			$$ = op4($1, NIL, makedfa(strnode($3), 1), $5, $7);
		  else
			$$ = op4($1, (Node *)1, $3, $5, $7); }
