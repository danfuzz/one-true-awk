...
#ifdef	_NFILE
#define	FILENUM	(_NFILE-5)	/* a guess about max number of open files */
#else
#define	FILENUM	15
#endif
...
Cell *matchop(a,n) Node **a;
{
	...
	if (n == MATCHFCN) {
		...
		if (patlen < 0)
			start = 0;
		setfval(rstloc, (Awkfloat) start);
		setfval(rlnloc, (Awkfloat) patlen);
		x = gettemp();
		x->tval = NUM;
		x->fval = start;
		return x;
	...
	while (*s) {
		...
		for (t=fmt; (*t++ = *s) != '\0'; s++) {
			if (islower(*s) && *s != 'l')
				break;
			if (*s == '*')
				error(FATAL, "in %s: %%*... conversion not supported.  try concatenation.", os);
		}
		*t = '\0';
		if (t >= fmt + sizeof(fmt))
...
Cell *split(a,nnn) Node **a;
{
	...
	if (a[2] == 0)		/* fs string */
		...
		error(FATAL, "illegal type of split()");
	sep = *fs;
	ap = execute(a[1]);	/* array name */
	freesymtab(ap);
...
Cell *bltin(a,n) Node **a;
{
	...
	case FSYSTEM:
		...
		break;
	case FRAND:
		u = (Awkfloat) (rand() % 32767) / 32767.0; break;
	case FSRAND:
		if (x->tval & REC)
			srand(time((long *)0));
		else
			srand((long)getfval(x));
		u = 0;
		break;
	default:
...
Cell *sub(a, nnn) Node **a;
{
	...
	if (pmatch(pfa, t)) {
		...
		*pb = '\0';
		if (pb >= buf + RECSIZE)
			error(FATAL, "sub() result %.20s too big", buf);
		tempfree(y);
		sptr = patbeg + patlen;
		...
		do {
			...
			*/
			if (patlen == 0 && *patbeg != 0) {	/* matched empty string */
				...
				*pb++ = *t++;
				if (pb >= buf + RECSIZE)
					error(FATAL, "gsub() result %.20s too big", buf);
				mflag = 0;
			}
